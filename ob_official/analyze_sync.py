...
def group_synced_frames(devices, ts_source, half_gap_us):
    ...
    failed is a list of dropped anchor (dev, FrameRecord) pairs."""
    ...
    while True:
        ...
        group = {}
        for i in range(n):
            # frames skipped between ptr[i] and match[i] were dropped from this device
...
def write_failed_csv(path, failed):
    """One row per dropped frame (unmatched anchor or skipped during a match)."""
        ...
        if depth and color and depth.frames and color.frames:
            ...
            if groups:
                if depth.is_ts_valid("global") and color.is_ts_valid("global"):
                    ...
                    lines.append("        global diff     : %d ~ %d us" % (min(g_diffs), max(g_diffs)))
                else:
                ...
                if src == "device" and device_valid:
                    ...
                    lines.append("        device diff     : %d ~ %d us" % (min(d_diffs), max(d_diffs)))
...
def analyze_sensor(sensor, devices_map, ts_source, half_gap_us, threshold, lines):
    ...
    lines.append("    Completeness      : %.1f%% (%d groups, %d drops)" % (completeness, len(groups), len(failed)))
...
def main():
    ...
    for sensor in ("depth", "color"):
        ...
        if failed:
            ...
            lines.append("    Failed CSV: %s (%d drops)" % (fail_csv, len(failed)))
        if groups:
